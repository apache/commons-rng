...
class BaseProviderTest {
    ...
    void testEnsureSeedLengthLong(int length) {
        ...
        for (long[] seed : new long[][] {
            ...
            Assertions.assertArrayEquals(expandSeed(length, seed),
                                         BaseProvider.ensureSeedLength(seed, length));
        }
    }
    ...
    /**
     * Test a seed can be expanded to a required size by filling with a SplitMix64-style
    ...
    @ParameterizedTest
    @ValueSource(ints = {0, 1, 2, 4, 5, 6, 7, 8, 9})
    void testEnsureSeedLengthInt(int length) {
